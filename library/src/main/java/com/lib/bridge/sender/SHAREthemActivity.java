...
import com.lib.bridge.qrcode.DetailsActivity;
import com.lib.bridge.qrcode.UpdateView;
import com.lib.bridge.qrcode.WifiQR;
import com.lib.bridge.utils.DividerItemDecoration;
...
import com.lib.bridge.utils.WifiUtils;
...
import static com.lib.bridge.sender.SHAREthemActivity.ShareUIHandler.LIST_API_CLIENTS;
import static com.lib.bridge.sender.SHAREthemActivity.ShareUIHandler.UPDATE_AP_STATUS;
import static com.lib.bridge.utils.Utils.DEFAULT_PORT_OREO;
import static com.lib.bridge.utils.Utils.getRandomColor;
import static com.lib.bridge.utils.Utils.isOreoOrAbove;
 ...
 */
public class SHAREthemActivity extends AppCompatActivity implements UpdateView {
    ...
    TextView m_showShareList, status, wifi_ssid, wifi_pass,browser_url;
    Toolbar m_toolbar;

    MaterialButton view_files;
    ...
    private MaterialCardView crd_hotspot_info, crd_files;

    private ImageView ch_init;
    private ProgressBar pg_init;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        view_files = findViewById(R.id.view_files);
        ...
        pg_init = findViewById(R.id.pg_init);
        ch_init = findViewById(R.id.ch_init);
        wifi_ssid = findViewById(R.id.wifi_ssid);
        wifi_pass = findViewById(R.id.wifi_pass);
        browser_url = findViewById(R.id.browser_url);
        ...
        crd_files = findViewById(R.id.crd_files);
        crd_files.setVisibility(View.INVISIBLE);
        crd_hotspot_info = findViewById(R.id.crd_hotspot_info);
        crd_hotspot_info.setVisibility(View.INVISIBLE);
    ...
    private void enableAp() {
        status.setText(getString(R.string.p2p_sender_hint_connecting));
    ...
    public void showQr(String qrString, String Type) {
        ...
        Intent intent =  new Intent(SHAREthemActivity.this, DetailsActivity.class);
    ...
    private void updateApStatus() {
        if (!HotspotControl.isSupported()) {
            status.setText("Hotspot mode not supported.\n");
        }
        if (hotspotControl.isEnabled()) {
            ...
            //m_sender_wifi_info.setText("Turning on Hotspot.");
            final WifiConfiguration config = hotspotControl.getConfiguration();
            ...
            if (TextUtils.isEmpty(ip))
                ...
                ip = ip.replace("/", "");
            status.setText(getString(R.string.p2p_sender_subtitle));
            crd_hotspot_info.setVisibility(View.VISIBLE);
            crd_files.setVisibility(View.VISIBLE);
            wifi_ssid.setText("SSID: "+config.SSID);
            wifi_pass.setText("Password: "+ config.preSharedKey);
            browser_url.setText("http://" + ip + ":" + hotspotControl.getShareServerListeningPort());
            ...
            btn_qr.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    Bundle b = new Bundle();
                    b.putString("ssid", config.SSID);
                    b.putString("password", config.preSharedKey);
                    wifi_qr.setArguments(b);
                    fragmentTransaction.add(R.id.frameLayout, wifi_qr).commit();
                }
            });

            ch_init.setVisibility(View.VISIBLE);
            pg_init.setVisibility(View.GONE);
            //if (m_showShareList.getVisibility() == View.VISIBLE) {
                m_showShareList.setText("Shared files: "+m_sharedFilePaths.length);
    ...
    private void resetSenderUi(boolean disableAP) {
        m_uiUpdateHandler.removeCallbacksAndMessages(null);
        status.setText("Hotspot disabled");
        status.setText(getString(R.string.p2p_sender_hint_text));
